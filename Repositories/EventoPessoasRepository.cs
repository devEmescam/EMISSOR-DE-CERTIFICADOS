    ...
    internal class EventoPessoasRepository : IEventoPessoasRepository
    {
        ...
        private async Task<List<PessoaEvento>> CarregarPessoasEventoAsync(int idEvento, bool emitirCertificado)
        {
            ...
            {               
                ...
                sSQL = "SELECT EP.ID ID_EVENTO_PESSOA, EP.ID_PESSOA, P.NOME, EP.CERTIFICADO_EMITIDO, FORMAT(EP.DATA_EMISSAO,'dd/MM/yyyy') DATA_EMISSAO, EP.MENSAGEM_RETORNO_EMAIL, EP.TEXTO_FRENTE, P.CPF, P.EMAIL ";
                ...
                foreach (DataRow row in oDT.Rows)
                {
                    ...
                    {
                        IdPessoa = row["ID_PESSOA"] != DBNull.Value ? Convert.ToInt32(row["ID_PESSOA"]) : 0,
                        Nome = row["NOME"] != DBNull.Value ? Convert.ToString(row["NOME"]) : string.Empty,
                        ...
                        IdEventoPessoa = row["ID_EVENTO_PESSOA"] != DBNull.Value ? Convert.ToInt32(row["ID_EVENTO_PESSOA"]) : 0
