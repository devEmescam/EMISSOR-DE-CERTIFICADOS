...
namespace EMISSOR_DE_CERTIFICADOS.Controllers
{
    public class PessoaController : Controller
    {
        ...
        // VERSÃO ASYNC: Método assíncrono para inserir uma nova pessoa no banco de dados.
                ...
                if (!await ExistePessoaComCPFAsync(pessoa.CPF))
                {
                    if (Util.ValidaCPF(pessoa.CPF))
                    {
                        if (Util.ValidaEstruturaEmail(pessoa.Email))
                        {
                            ...
                            var sSQL = "INSERT INTO PESSOA (NOME, CPF, EMAIL, ID_USUARIO_ADMINISTRATIVO, DATA_CADASTRO) " +
                                       "VALUES (@Nome, @CPF, @Email, @idUsuario, GETDATE()); " +
                            ...
                            pessoa.Id = await _dbHelper.ExecuteScalarAsync<int>(sSQL, parameters);
